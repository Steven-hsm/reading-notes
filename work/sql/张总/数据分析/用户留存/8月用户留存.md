1.当月会员购买数

```sql
SELECT
c.channelName 渠道名称,
COUNT(DISTINCT a.id) 渠道新访问人数,
SUM(d.buyPeople) 当月购买人数,
SUM(d.tradeFunds) 当月购买金额
from
jkb_vip.user_into_log a
INNER JOIN (
	-- 获取第一次领取会员卡的数据
	SELECT min(id) id,wuId from jkb_vip.user_into_log a
	INNER JOIN jkb_vip.wx_union_user b on a.wxUnionId=b.wxUnionId
	GROUP BY b.wuId
) b on a.id = b.id
LEFT JOIN jkb_vip.channel_track_label c on a.sid=c.channelCode
-- 当月用户订单数据
LEFT JOIN (
	SELECT wuId,count(DISTINCT wuId) buyPeople,SUM(tradeFunds/100) tradeFunds from jkb_vip_core.orders
	where tradeFunds>0 and  payState=2 and FROM_UNIXTIME(ctime/1000,'%Y%m%d') BETWEEN 20190801 AND 20190831
GROUP BY wuId 
) d on b.wuId=d.wuId
where FROM_UNIXTIME(a.ctime/1000,'%Y%m%d') BETWEEN 20190801 AND 20190831
GROUP BY a.sid order by COUNT(DISTINCT a.id) desc,a.sid
```

2. 次月数据

```sql
SELECT
c.channelName 渠道名称,
COUNT(DISTINCT a.id) 渠道新访问人数,
COUNT(DISTINCT a1.wuid) 次月留存人数,
SUM(d.buyPeople) 当月购买人数,
SUM(d.tradeFunds) 当月购买金额
from
jkb_vip.user_into_log a
INNER JOIN (
	-- 获取第一次领取会员卡的数据
	SELECT min(id) id,wuId from jkb_vip.user_into_log a
	INNER JOIN jkb_vip.wx_union_user b on a.wxUnionId=b.wxUnionId
	GROUP BY b.wuId
) b on a.id = b.id
LEFT JOIN jkb_vip.channel_track_label c on a.sid=c.channelCode
LEFT JOIN(
	-- 获取第一次领取会员卡的数据
	SELECT b.wuId from jkb_vip.user_into_log a
	INNER JOIN jkb_vip.wx_union_user b on a.wxUnionId=b.wxUnionId
	where FROM_UNIXTIME(a.ctime/1000,'%Y%m%d') BETWEEN 20190901 AND 20190930
	GROUP BY b.wuId
) a1 on a1.wuId=b.wuId
-- 当月用户订单数据
LEFT JOIN (
	SELECT wuId,count(DISTINCT wuId) buyPeople,SUM(tradeFunds/100) tradeFunds from jkb_vip_core.orders
	where tradeFunds>0 and  payState=2 and FROM_UNIXTIME(ctime/1000,'%Y%m%d') BETWEEN 20190901 AND 20190930
GROUP BY wuId 
) d on b.wuId=d.wuId
where FROM_UNIXTIME(a.ctime/1000,'%Y%m%d') BETWEEN 20190801 AND 20190831
GROUP BY a.sid order by COUNT(DISTINCT a.id) desc,a.sid
```



3. 三月数据

```sql
SELECT
COUNT(DISTINCT a.wuId) 渠道新增用户,
COUNT(DISTINCT a1.wuId) 三月留存人数,
SUM(d1.buyPeople) 三月购买人数,
SUM(d1.tradeFunds) 三月购买金额
from
jkb_vip.vip_card_receive a
LEFT JOIN jkb_vip_core.orders mm on a.orderId=mm.orderId
INNER JOIN (
	-- 获取第一次领取会员卡的数据
	SELECT MIN(receiveId) receiveId FROM jkb_vip.vip_card_receive GROUP BY wuId
) b on a.receiveId = b.receiveId
-- 三月留存会员数
LEFT JOIN(
	SELECT wuId from jkb_vip.vip_card_receive where FROM_UNIXTIME(effectTime/1000,'%Y%m%d') <=20191031 and FROM_UNIXTIME(validTime/1000,'%Y%m%d')>=20191001 GROUP BY wuId
) a1 on a1.wuId=a.wuId
-- 三月用户订单数据
LEFT JOIN (
	SELECT wuId,count(DISTINCT wuId) buyPeople,SUM(tradeFunds/100) tradeFunds from jkb_vip_core.orders
	where tradeFunds>0 and  payState=2 and FROM_UNIXTIME(ctime/1000,'%Y%m%d') BETWEEN 20191001 AND 20191031 
GROUP BY wuId 
) d1 on a.wuId=d1.wuId

where FROM_UNIXTIME(a.drawTime/1000,'%Y%m%d') BETWEEN 20190801 AND 20190831
GROUP BY mm.channelCode order by COUNT(DISTINCT a.wuId) desc,a.channelCode desc


```

4. 四月

```sql
SELECT
COUNT(DISTINCT a.wuId) 渠道新增用户,
COUNT(DISTINCT a1.wuId) 四月留存人数,
SUM(d1.buyPeople) 四月购买人数,
SUM(d1.tradeFunds) 四月购买金额
from
jkb_vip.vip_card_receive a
LEFT JOIN jkb_vip_core.orders mm on a.orderId=mm.orderId
INNER JOIN (
	-- 获取第一次领取会员卡的数据
	SELECT MIN(receiveId) receiveId FROM jkb_vip.vip_card_receive GROUP BY wuId
) b on a.receiveId = b.receiveId
-- 四月留存会员数
LEFT JOIN(
	SELECT wuId from jkb_vip.vip_card_receive where FROM_UNIXTIME(effectTime/1000,'%Y%m%d') <=20191130 and FROM_UNIXTIME(validTime/1000,'%Y%m%d')>=20191101 GROUP BY wuId
) a1 on a1.wuId=a.wuId
-- 四月用户订单数据
LEFT JOIN (
	SELECT wuId,count(DISTINCT wuId) buyPeople,SUM(tradeFunds/100) tradeFunds from jkb_vip_core.orders
	where tradeFunds>0 and  payState=2 and FROM_UNIXTIME(ctime/1000,'%Y%m%d') BETWEEN 20191101 AND 20191130 
GROUP BY wuId 
) d1 on a.wuId=d1.wuId

where FROM_UNIXTIME(a.drawTime/1000,'%Y%m%d') BETWEEN 20190801 AND 20190831
GROUP BY mm.channelCode order by COUNT(DISTINCT a.wuId) desc,a.channelCode desc

```



5. 五月

```sql
SELECT
COUNT(DISTINCT a.wuId) 渠道新增用户,
COUNT(DISTINCT a1.wuId) 五月留存人数,
SUM(d1.buyPeople) 五月购买人数,
SUM(d1.tradeFunds) 五月购买金额
from
jkb_vip.vip_card_receive a
INNER JOIN (
	-- 获取第一次领取会员卡的数据
	SELECT MIN(receiveId) receiveId FROM jkb_vip.vip_card_receive GROUP BY wuId
) b on a.receiveId = b.receiveId
-- 五月留存会员数
LEFT JOIN(
	SELECT wuId from jkb_vip.vip_card_receive where FROM_UNIXTIME(effectTime/1000,'%Y%m%d') <=20191231 and FROM_UNIXTIME(validTime/1000,'%Y%m%d')>=20191201 GROUP BY wuId
) a1 on a1.wuId=a.wuId
-- 五月用户订单数据
LEFT JOIN (
	SELECT wuId,count(DISTINCT wuId) buyPeople,SUM(tradeFunds/100) tradeFunds from jkb_vip_core.orders
	where tradeFunds>0 and  payState=2 and FROM_UNIXTIME(ctime/1000,'%Y%m%d') BETWEEN 20191201 AND 20191231
GROUP BY wuId 
) d1 on a.wuId=d1.wuId

where FROM_UNIXTIME(a.drawTime/1000,'%Y%m%d') BETWEEN 20190801 AND 20190831
GROUP BY a.channelCode order by COUNT(DISTINCT a.wuId) desc,a.channelCode desc

```